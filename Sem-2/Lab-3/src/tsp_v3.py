...
filePath = "data/graph_5v.dat"		# Path to the file with
...
def brutalTSP(graphData, startPoint, citiesNum):
	brute = True
	currDist = 0
	visitedNum = 0
	currCity = startPoint
	...
	while (brute):
		...
		if (currDist < shortestPath) and (visitedNum == citiesNum):
			shortestPath = currDist

def main():
	graphData = loadGraph()
	startPoint = endPoint = rnd(0, len(graphData)-1)
	citiesNum = len(graphData)
	brutalTSP(graphData, startPoint, citiesNum)
